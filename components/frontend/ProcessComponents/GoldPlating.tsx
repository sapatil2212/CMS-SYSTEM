...
const GoldPlating = () => {
  ...
  const fetchContent = async () => {
    try {
      const response = await fetch('/api/content/gold-plating')
      if (response.ok) {
  ...
  const getDefaultContent = () => ({
    ...
    benefits: [
      { icon: "Crown", title: "Corrosion Resistance", description: "Unmatched resistance to oxidation and tarnishing" },
      { icon: "Zap", title: "Electrical Conductivity", description: "Excellent conductivity for high-frequency applications" },
      { icon: "Shield", title: "Chemical Stability", description: "Resistant to most acids and alkalis" },
      { icon: "Award", title: "Aesthetic Appeal", description: "Beautiful finish for decorative applications" },
      { icon: "CircuitBoard", title: "Contact Reliability", description: "Maintains low contact resistance over time" },
      { icon: "TestTube2", title: "Biocompatibility", description: "Safe for medical and dental applications" }
    ],
    processSteps: [
      ...
      { step: "2", title: "Nickel Undercoat", description: "Deposition of nickel barrier layer", icon: "M19 11a7 7 0 01-7 7m0 0a7 7 0 01-7-7m7 7v4m0 0H8m4 0h4m-4-8a3 3 0 01-3-3V5a3 3 0 116 0v6a3 3 0 01-3 3z" },
      { step: "3", title: "Gold Deposition", description: "Precise DC current deposits gold layer", icon: "M13 10V3L4 14h7v7l9-11h-7z" },
    ...
    applications: [
      { title: "Electronics & PCBs", image: "/uploads/gold-plating/electronics.jpg", items: ["Circuit boards", "Connectors", "RF components", "Contact points"] },
      { title: "Aerospace & Defense", image: "/uploads/gold-plating/aerospace.jpg", items: ["Critical components", "Electrical systems", "Corrosion protection", "High-reliability parts"] },
      { title: "Medical Devices", image: "/uploads/gold-plating/medical.jpg", items: ["Surgical instruments", "Dental equipment", "Implantable devices", "Diagnostic tools"] }
    ],
    industries: [
      { name: "Electronics", icon: "Cpu", examples: ["PCBs", "Connectors", "RF components"], image: "/uploads/gold-plating/electronics-industry.jpg" },
      { name: "Aerospace", icon: "Plane", examples: ["Critical components", "Electrical systems", "Corrosion protection"], image: "/uploads/gold-plating/aerospace-industry.jpg" },
      { name: "Medical", icon: "Shield", examples: ["Surgical instruments", "Dental equipment", "Implantable devices"], image: "/uploads/gold-plating/medical-industry.jpg" }
    ],
    qualityChecks: [
      ...
      { title: "Corrosion Testing", description: "Salt spray and humidity testing" },
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-4 text-gray-600">Loading Gold Plating content...</p>
        </div>
      </div>
    )
  }
                  ...
                  <ul className="space-y-2">
                    {content.whatIsBestFor.split('\n').map((item: string, index: number) => (
                      <li key={index} className="flex items-center text-gray-700">
                        <Check className="h-4 w-4 text-yellow-600 mr-2 flex-shrink-0" />
                        {item}
                      </li>
                    ))}
                  </ul>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {content.processSteps.map((step: any, index: number) => (
              <div key={index} className="text-center">
                <div className="bg-yellow-600 text-white rounded-full w-16 h-16 flex items-center justify-center mx-auto mb-4 text-xl font-bold">
                  {step.step}
                </div>
                <h3 className="text-lg font-semibold text-gray-900 mb-2">
                  {step.title}
