...
export async function GET() {
  try {
    const content = await prisma.aluminiumContent.findFirst({
      include: {
...
export async function POST(request: NextRequest) {
  try {
    ...
    const existingContent = await prisma.aluminiumContent.findFirst()
    
    if (existingContent) {
      // Update existing content
      const updatedContent = await prisma.aluminiumContent.update({
      ...
      // Create new content
      const newContent = await prisma.aluminiumContent.create({
        data: {
          heroTitle: body.heroTitle,
          heroSubtitle: body.heroSubtitle,
          heroDescription: body.heroDescription,
    ...
    console.error('Error saving aluminium content:', error)
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 })
  }
} 