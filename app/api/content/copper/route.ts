...
export async function GET() {
  try {
    const content = await prisma.copperContent.findFirst({
      include: {
    ...
    console.error('Error fetching copper content:', error)
...
export async function POST(request: NextRequest) {
  try {
    ...
    const existingContent = await prisma.copperContent.findFirst()
    
    if (existingContent) {
      // Update existing content
      const updatedContent = await prisma.copperContent.update({
      ...
      // Create new content
      const newContent = await prisma.copperContent.create({
        data: {
          heroTitle: body.heroTitle || '',
          heroSubtitle: body.heroSubtitle || '',
          heroDescription: body.heroDescription || '',
    ...
    console.error('Error saving copper content:', error)
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 })
  }
} 