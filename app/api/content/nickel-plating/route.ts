...
export async function GET() {
  try {
    const content = await prisma.nickelPlatingContent.findFirst({
      include: {
        benefits: {
          orderBy: { order: 'asc' }
        },
    ...
    console.error('Error fetching nickel plating content:', error)
...
export async function POST(request: NextRequest) {
  try {
    ...
    const existingContent = await prisma.nickelPlatingContent.findFirst()
    
    if (existingContent) {
      // Update existing content
      const updatedContent = await prisma.nickelPlatingContent.update({
      ...
      // Create new content
      const newContent = await prisma.nickelPlatingContent.create({
        data: {
          heroTitle: body.heroTitle,
          heroSubtitle: body.heroSubtitle,
          heroDescription: body.heroDescription,
    ...
    console.error('Error saving nickel plating content:', error)
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 })
  }
} 