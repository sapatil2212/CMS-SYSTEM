...
export async function GET() {
  try {
    const content = await prisma.goldPlatingContent.findFirst({
      include: {
        benefits: {
          orderBy: { order: 'asc' }
        },
    ...
    console.error('Error fetching gold plating content:', error)
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (existingContent) {
      ...
      const updatedContent = await prisma.goldPlatingContent.update({
      ...
      // Create new content
      const newContent = await prisma.goldPlatingContent.create({
        data: {
          heroTitle: body.heroTitle,
          heroSubtitle: body.heroSubtitle,
          heroDescription: body.heroDescription,
    ...
    console.error('Error saving gold plating content:', error)
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 })
  }
} 