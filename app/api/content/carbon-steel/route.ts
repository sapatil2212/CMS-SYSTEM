...
export async function GET() {
  try {
    const content = await prisma.carbonSteelContent.findFirst({
      include: {
    ...
    console.error('Error fetching carbon steel content:', error)
...
export async function POST(request: NextRequest) {
  try {
    ...
    const existingContent = await prisma.carbonSteelContent.findFirst()
    
    if (existingContent) {
      // Update existing content
      const updatedContent = await prisma.carbonSteelContent.update({
      ...
      // Create new content
      const newContent = await prisma.carbonSteelContent.create({
        data: {
          heroTitle: body.heroTitle || '',
          heroSubtitle: body.heroSubtitle || '',
          heroDescription: body.heroDescription || '',
    ...
    console.error('Error saving carbon steel content:', error)
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 })
  }
}