...
export async function GET() {
  try {
    const content = await prisma.molykoteContent.findFirst({
      include: {
        benefits: {
          orderBy: { order: 'asc' }
        },
    ...
    console.error('Error fetching molykote content:', error)
...
export async function POST(request: NextRequest) {
  try {
    ...
    const existingContent = await prisma.molykoteContent.findFirst()
    
    if (existingContent) {
      // Update existing content
      const updatedContent = await prisma.molykoteContent.update({
      ...
      // Create new content
      const newContent = await prisma.molykoteContent.create({
        data: {
          heroTitle: body.heroTitle,
          heroSubtitle: body.heroSubtitle,
          heroDescription: body.heroDescription,
    ...
    console.error('Error saving molykote content:', error)
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 })
  }
} 