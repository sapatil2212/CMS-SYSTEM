...
export async function GET() {
  try {
    const content = await prisma.silverPlatingContent.findFirst({
      include: {
        benefits: {
          orderBy: { order: 'asc' }
        },
    ...
    console.error('Error fetching silver plating content:', error)
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (existingContent) {
      ...
      const updatedContent = await prisma.silverPlatingContent.update({
      ...
      // Create new content
      const newContent = await prisma.silverPlatingContent.create({
        data: {
          heroTitle: body.heroTitle,
          heroSubtitle: body.heroSubtitle,
          heroDescription: body.heroDescription,
    ...
    console.error('Error saving silver plating content:', error)
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 })
  }
} 