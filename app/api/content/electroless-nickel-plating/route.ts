...
export async function GET() {
  try {
    const content = await prisma.electrolessNickelPlatingContent.findFirst({
      include: {
        benefits: {
          orderBy: { order: 'asc' }
        },
    ...
    console.error('Error fetching electroless nickel plating content:', error)
...
export async function POST(request: NextRequest) {
  try {
    ...
    const existingContent = await prisma.electrolessNickelPlatingContent.findFirst()
    
    if (existingContent) {
      // Update existing content
      const updatedContent = await prisma.electrolessNickelPlatingContent.update({
      ...
      // Create new content
      const newContent = await prisma.electrolessNickelPlatingContent.create({
        data: {
          heroTitle: body.heroTitle,
          heroSubtitle: body.heroSubtitle,
          heroDescription: body.heroDescription,
    ...
    console.error('Error saving electroless nickel plating content:', error)
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 })
  }
} 