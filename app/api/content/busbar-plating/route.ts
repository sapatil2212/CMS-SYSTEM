...
export async function GET() {
  try {
    const content = await prisma.busbarPlatingContent.findFirst({
      include: {
        benefits: {
          orderBy: { order: 'asc' }
        },
    ...
    console.error('Error fetching busbar plating content:', error)
...
export async function POST(request: NextRequest) {
  try {
    ...
    const existingContent = await prisma.busbarPlatingContent.findFirst()
    
    if (existingContent) {
      // Update existing content
      const updatedContent = await prisma.busbarPlatingContent.update({
      ...
      // Create new content
      const newContent = await prisma.busbarPlatingContent.create({
        data: {
          heroTitle: body.heroTitle,
          heroSubtitle: body.heroSubtitle,
          heroDescription: body.heroDescription,
    ...
    console.error('Error saving busbar plating content:', error)
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 })
  }
} 