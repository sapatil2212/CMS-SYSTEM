...
export async function GET() {
  try {
    const content = await prisma.brassContent.findFirst({
      include: {
...
export async function POST(request: NextRequest) {
  try {
    ...
    const existingContent = await prisma.brassContent.findFirst()
    
    if (existingContent) {
      // Update existing content
      const updatedContent = await prisma.brassContent.update({
      ...
      // Create new content
      const newContent = await prisma.brassContent.create({
        data: {
          heroTitle: body.heroTitle || '',
          heroSubtitle: body.heroSubtitle || '',
          heroDescription: body.heroDescription || '',
    ...
    console.error('Error saving brass content:', error)
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 })
  }
} 